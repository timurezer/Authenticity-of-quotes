...
def add(h: list, sz: list, x: tuple, d):
    h.append(x)
    sz[0] += 1
    i = siftUp(h, sz, d)
    return i
...
def siftUp(h, sz, d):
    ...
    while cur > 0 and h[cur][1] < h[parent][1]: # .. and ( ....  or (h[cur][1] == h[parent][1] and h[cur][0] < h[parent][0])):
        swap(h, cur, parent, d)
        cur = parent
        parent = (cur - 1) // 2
    return cur
...
def siftDown(h, sz, i, d):
    ...
    while child < sz[0]:  # тут могло сломаться при обращении, если нет child выходит за границы, но все окей...
        ...
        if child + 1 < sz[0] and (h[child + 1][1] < h[child][1] or (h[child + 1][1] == h[child][1] and h[child + 1][0] < h[child][0])):    # и здесь
            child += 1
        if h[cur][1] < h[child][1] or (h[cur][1] == h[child][1] and h[cur][0] < h[child][0]):   # спомни как ты тут обосрался
            break
        swap(h, cur, child, d)
        cur = child
        child = 2*cur + 1
    ...
    while request:
        ...
        if request[1] == '+':
            ...
            if x == -1:     # all cells are free
                ...
                siftDown(h, sz, i, d)      # new position in heap, update d
