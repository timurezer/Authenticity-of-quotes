...
def merge_and_count(a, n, b, m):
    ...
    while i < n and j < m:
        if a[i] <= b[j]:
            ...
            j += 1
            b_cur_numb += 1
    res += a[i:]; inv_numb += (n - i)*b_cur_numb
    res += b[j:]
    return res, inv_numb

def mergesort_rec_inv_count(s: list, n: int):
    if n <= 1:
        return s, 0
    n_new = n//2
    a, inv_numb1 = mergesort_rec_inv_count(s[0: n_new], n_new)
    b, inv_numb2 = mergesort_rec_inv_count(s[n_new:], n - n_new)
    s, add_inv = merge_and_count(a, n_new, b, n - n_new)
    return s, add_inv + inv_numb1 + inv_numb2
...
def main():
    ...
    sorted_list, total_inv = mergesort_rec_inv_count(s, n)
